...
def parse_args():
    ...
    parser.add_argument('stock_code', type=str, help='股票代码，例如：sz000002')
...
# 2. 准备发送给大模型的分析提示
def prepare_analysis_prompt(df, stock_code):
    """准备分析提示词"""
    # 获取最新数据
...
def main():
    try:
        ...
        print(f"正在获取{stock_code}近三年数据...")
        df = get_stock_data(stock_code, start_date, end_date)
        print(f"成功获取{len(df)}条数据")
        print("正在准备分析提示...")
        prompt = prepare_analysis_prompt(df, stock_code)
        print("正在调用大模型进行分析...")
        ...
        filename = f"{stock_code}_{today}_analysis_report.txt"
        with open(filename, "w", encoding="utf-8") as f:
            f.write(analysis_result)
            f.flush()          # 强制把缓冲区刷到磁盘
            os.fsync(f.fileno())  # 让操作系统立即落盘（Windows 也生效）
        print("写入内容长度:", len(analysis_result))
        print(f"\n分析报告已保存至 {filename}")
